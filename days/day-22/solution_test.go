...
func TestGetCubesInCuboid(t *testing.T) {
	tests := []struct {
		step string
		want []Cube
	}{
		...
		{
			step: "on x=10..10,y=10..10,z=10..10",
			want: []Cube{
				{10, 10, 10},
			},
		},
	...
	for _, tc := range tests {
		step := RebootStepFromStr(tc.step)
		r := &Reactor{}
		got := r.getCubesInCuboid(step.cuboid)
